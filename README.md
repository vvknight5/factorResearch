...
基于 [Polars](https://pola.rs/) 的 A 股因子研究工具包：预处理、非线性因子线性化、正交化、IC 评价、因子相关、分组回测与可视化。
...
# 3. 评价与回测（过滤 null 样本）
valid = panel.filter(pl.col("new_factor_orth").is_not_null())
ic_table = evaluate_factor(valid, factor_col="new_factor_orth", ret_col="fut_ret")
annual_ic = evaluate_factor_by_period(
    valid, period="year", factor_col="new_factor_orth", ret_col="fut_ret",
)
daily_ic = plot_factor_ic(valid, factor_col="new_factor_orth", ret_col="fut_ret", show=False)
perf = factor_group_backtest(valid, n=10, factor_col="new_factor_orth", ret_col="fut_ret", show=False)

# 4. 多因子相关与可视化
corr = factor_correlation_matrix(valid, ["raw_factor", "new_factor", "new_factor_orth"])
